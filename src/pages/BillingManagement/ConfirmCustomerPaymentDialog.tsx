...
import { Autocomplete, Button, Dialog, DialogActions, DialogContent, DialogTitle, FormControl, FormControlLabel, FormLabel, Grid, Radio, RadioGroup, Stack, TextField, Typography } from '@mui/material';
...
import { deleteSaleOrderPackImage } from 'services/SaleOrder/sale-order-api';
...
export default function ConfirmCustomerPaymentDialog(props: ConfirmCustomerPaymentDialogProps): JSX.Element {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            toast.promise(confirmBillingNotePayment(billingNote?.id, formData), {
    ...
    const outstandingAmount = useMemo(() => {
        if (!billingNote || billingNote.billingNoteItems.length === 0) return 0;

        return billingNote.totalAmount - billingNote.paidAmount;
    }, [billingNote]);
    ...
    useEffect(() => {
        if (!open || !billingNote || billingNote.billingNoteItems.length === 0) return;

        (async () => {
            const allSlipFiles: File[] = [];

            for (const slip of billingNote.picUrls || []) {
                const file = await urlToFile(slip, `${billingNote.id}`);
                (file as any).isOriginal = true;
                (file as any).billingId = billingNote.id; // (optional) ไว้ trace

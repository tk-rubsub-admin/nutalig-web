...
import { downloadBilling } from 'services/Billing/billing-api';
import { BillingNoteDto } from 'services/Billing/billing-type';
import { base64ToBlob } from 'utils';

export interface ViewBillingDialogProps {
    open: boolean;
    url: string;
    billingNo: string;
...
export default function ViewBillingDialog(props: ViewBillingDialogProps): JSX.Element {
    const { open, url, billingNo, billingNote, options, onClose } = props;
    ...
    const downloadBillingNoteFunction = (billing: BillingNoteDto, opt: string) => {
    ...
    return (
        <Dialog open={open} maxWidth="sm" fullWidth aria-labelledby="form-dialog-title">
            <DialogTitle id="form-dialog-title">
                {t('billingManagement.viewBillingTitle', { billingNo })}
            </DialogTitle>
            <DialogContent>
                <iframe
                    ref={(el) => {
            ...
            <DialogActions>
                <>
                    ...
                    <Menu
                        ...
                        keepMounted>
                        <MenuItem onClick={() => downloadBillingNoteFunction(billingNote, 'PDF')}>
                            <ListItemIcon>
                        ...
                        <MenuItem onClick={() => downloadBillingNoteFunction(billingNote, 'JPG')}>
                            <ListItemIcon>

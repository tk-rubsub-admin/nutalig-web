...
import { ArrowBack, Edit, Launch, Preview } from "@mui/icons-material";
...
import { getInvoiceGroup, viewInvoiceGroup } from "services/Invoice/invoice-group-api";
import { InvoiceGroupItem } from "services/Invoice/invoice-type";
import ViewInvoiceDialog from "pages/InvoiceManagement/ViewInvoiceDialog";
...
export interface InvoiceGroupParam {
    id: string;
}
...
export default function InvoiceGroupDetail() {
    ...
    const params = useParams<InvoiceGroupParam>();
    ...
    const [invoiceGroupItems, setInvoiceGroupItems] = useState<InvoiceGroupItem[]>([]);
    ...
    const {
        ...
        isFetching: isInvoiceGroupFetching
    } = useQuery(
        ['invoice-group-detail', params.id],
        () => getInvoiceGroup(params.id),
        {
            enabled: !!params.id,
            refetchOnWindowFocus: false
        }
    ...
    const memoSubtotal = useMemo(() => {
        return invoiceGroupData?.subtotal;
    }, [invoiceGroupData]);
    ...
    const viewInvoiceGroupFunction = (id: string, options: { original: boolean; copy: boolean }) => {
        toast.promise(viewInvoiceGroup(id, options.original, options.copy), {
            loading: t('toast.loading'),
            success: (response) => {
    ...
    useEffect(() => {
        if (!invoiceGroupData) return;

        setCustomer(invoiceGroupData.customer);
        setInvoiceNo(invoiceGroupData.invoiceNo);
        setInvoiceGroupItems(invoiceGroupData.items);

        setApiSubtotal(invoiceGroupData.subtotal);
        setApiDiscount(invoiceGroupData.discount ?? 0);
        setDiscountInput(String(invoiceGroupData.discount ?? 0));
        setEnableWithHoldingTax(invoiceGroupData.withHoldingTaxFlag);
        setWithHoldingPercent(invoiceGroupData.withHoldingPercent ?? 3);

        // 4. set formik
        formik.setValues({
            invoiceDate: dayjs(invoiceGroupData.invoiceDate),
            dueDate: dayjs(invoiceGroupData.dueDate),
            customerId: invoiceGroupData.customer?.customerId,
            remark: invoiceGroupData.remark ?? '',
            creditDays: invoiceGroupData.creditDays,
            items: invoiceGroupData.items ?? []
        });
    ...
    return (
        <Page>
            <PageTitle title={t('invoiceManagement.group.viewInvoiceGroupTitle', { invoiceNo: invoiceNo })}>
                <Chip
                    color="info"
                    label={t(`status.invoice.${invoiceGroupData?.invoiceStatus}`)}
                    sx={{ ...getStatusChipSx(invoiceGroupData?.invoiceStatus), }}
                />
            </PageTitle>
            <Wrapper>
                <Stack
                ...
                >
                    ...
                    {invoiceGroupData?.status != 'CANCELLED' ?
                        <Button
                            fullWidth={isDownSm}
                            disabled={invoiceGroupData?.status === 'CANCELLED'}
                            onClick={() => {
                            ...
                            {t('invoiceManagement.group.viewInvoice')}
                        </Button>
                        : <></>}
                </Stack>
            </Wrapper>
                            ...
                            <TextField
                                ...
                                value={
                                    formik.values.invoiceDate
                                        ? dayjs(formik.values.invoiceDate).format(DEFAULT_DATE_FORMAT)
                                        : '-'
                                }
                                InputProps={{
                                    readOnly: true,
                                        ...
                                        <TableCell align="left">
                                            <Stack direction="row" alignItems="center" spacing={1}>
                                                ...
                                                <IconButton
                                                    size="small"
                                                    onClick={() =>
                                                        window.open(`/invoice/${item.invoice.invoiceNo}`, "_blank")
                                                    }
                                                >
            ...
            <ViewInvoiceDialog
                open={openViewDialog}
                url={pdfUrl}
                invNo={invoiceGroupData?.invoiceNo}
                invoice={invoiceGroupData}
                options={{ original: printOriginal, copy: printCopy }}
                type="INVOICE_GROUP"
                onClose={() => setOpenViewDialog(false)}
            />
            <LoadingDialog
                open={isInvoiceGroupFetching}
            />
            <ViewOptionDialog
                open={openViewOptionDialog}
                title={t('invoiceManagement.group.viewInvoiceGroupTitle', { invoiceNo: invoiceGroupData.invoiceNo })}
                onClose={() => {
                    setOpenViewOptionDialog(false)
                }}
                onConfirm={(options) => {
                    if (invoiceGroupData) {
                        viewInvoiceGroupFunction(invoiceGroupData.invoiceNo, options);
                    }

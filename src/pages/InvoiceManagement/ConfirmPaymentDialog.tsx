...
import { confirmPaidSaleOrder, deleteSaleOrderPackImage } from 'services/SaleOrder/sale-order-api';
...
import { confirmPaidInvoice } from 'services/Invoice/invoice-api';
...
export default function ConfirmPaymentDialog(props: ConfirmPaymentDialogProps): JSX.Element {
    ...
    const formik = useFormik({
        initialValues: {
            amount: '',
            slipFiles: [], // ใช้สำหรับ validate เท่านั้น ไม่เก็บจริง
            paymentChannel: '',
            bankAccount: null,
            creditEnabled: false,
        ...
        onSubmit: async (values) => {
            ...
            formData.append('bankAccount', values.paymentChannel === 'TRANSFER' ? values.bankAccount.code : '');

            toast.promise(confirmPaidInvoice(inv?.invoiceNo, formData), {
    ...
    useEffect(() => {
        ...
        (async () => {
            const slipFiles = await Promise.all(
                (inv.slips || []).map(async (pic) => {
                    const file = await urlToFile(pic.picUrl, `${pic.picId}`);
                    // mark as original file (already uploaded before)
                    (file as any).isOriginal = true;
                    return file;
                })
            );
            setSlipImageFiles(slipFiles);
            formik.setFieldValue('slipFiles', slipFiles);
            if (inv.paymentChannel !== null || inv.bankAccount !== null) {
                setDisableChannel(true);
            }
            formik.setFieldValue('paymentChannel', inv.paymentChannel);
            originalSlipFilesRef.current = slipFiles; // snapshot originals

            const config = await getSystemConfig(GROUP_CODE.BANK_ACCOUNT); // เปลี่ยน groupCode ตามที่ใช้จริง
            setSystemConfig(config);

            const selectedBankAccount =
                config.find((c) => c.code === inv.bankAccount) || null;

            formik.setFieldValue('bankAccount', selectedBankAccount);

            const credit = customer?.creditBalance;
    ...
    }, [slipImageFiles]); // cleanup when list changes/unmounts
    ...
    return (
        <Dialog open={open} maxWidth="xs" fullWidth aria-labelledby="form-dialog-title">
            <DialogTitle
                ...
                {t('invoiceManagement.confirmPaymentDialog.title', { poId: inv?.poId })}

                <ManualHelpButton manualId="MANUAL000012" />
            </DialogTitle>
            <DialogContent>
                <Grid container spacing={1}>
                    <GridTextField item xs={12} sm={12}>
                        ...
                        <ImageFileUploaderWrapper
                            ...
                            onError={() => { }}
                            onDeleted={(index) => {
                                const deletedFile = slipImageFiles.filter((_, i) => i === index)[0];
                                toast.promise(deleteSaleOrderPackImage(inv?.poId, deletedFile.name), {
                                    loading: t('toast.loading'),
                                    success: () => {
                                        setSlipImageFiles(prev => prev.filter((_, i) => i !== index));
                                        formik.setFieldValue('slipFiles',
                    ...
                    <Grid item xs={6} sm={6}>
                        <TextField
                            type="text"
                            label={t('invoiceManagement.column.transferAmount')}
                            fullWidth
                            variant="outlined"
                            value={inv ? formatMoney(inv.invoiceAmount) : '-'}
                    ...
                    <Grid item xs={6} sm={6}>
                        <TextField
                            ...
                            fullWidth
                            variant="outlined"
                            value={inv ? formatMoney(inv.poAmount - inv.invoiceAmount) : '-'}
                            InputLabelProps={{ shrink: true }}
                        />
                    </Grid>
                    ...
                    {formik.values.paymentChannel === 'TRANSFER' ? (
                        <GridTextField item xs={12} sm={12}>
                            <Autocomplete
                                disabled={systemConfig.length === 0 || disableChannel}
                                disablePortal
            ...
            <DialogActions>
                ...
                <Button
                    onClick={() => {
                        setTitle(t('invoiceManagement.confirmPaymentDialog.title', { poId: inv?.poId }));

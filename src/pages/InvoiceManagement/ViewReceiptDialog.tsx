...
import { Button, Dialog, DialogActions, DialogContent, DialogTitle, Divider, ListItemIcon, ListItemText, Menu, MenuItem } from '@mui/material';
import { useState } from 'react';
import toast from 'react-hot-toast';
import { useTranslation } from 'react-i18next';
import { Invoice } from 'services/Invoice/invoice-type';
import { downloadReceipt, downloadReceiptListV1 } from 'services/SaleOrder/sale-order-api';
import { base64ToBlob } from 'utils';

export interface ViewReceiptDialogProps {
    open: boolean;
    url: string;
    receiptNos: string[];
    invoice?: Invoice;
...
export default function ViewReceiptDialog(props: ViewReceiptDialogProps): JSX.Element {
    const { open, url, receiptNos, invoice, options, onClose } = props;
    ...
    const downloadReceiptFunction = (inv: Invoice, format: string) => {
        toast.promise(downloadReceiptListV1(inv.poId, format, options.original, options.copy), {
    ...
    return (
        <Dialog open={open} maxWidth="sm" fullWidth aria-labelledby="form-dialog-title">
            <DialogTitle id="form-dialog-title">
                {t('invoiceManagement.receiptNoTitle')}
            </DialogTitle>
            <DialogContent>
                <iframe
                    ref={(el) => {
            ...
            <DialogActions>
                <>
                    ...
                    <Menu
                        ...
                        keepMounted>
                        <MenuItem onClick={() => downloadReceiptFunction(invoice, 'PDF')}>
                            <ListItemIcon>
                        ...
                        <MenuItem onClick={() => downloadReceiptFunction(invoice, 'JPG')}>
                            <ListItemIcon>

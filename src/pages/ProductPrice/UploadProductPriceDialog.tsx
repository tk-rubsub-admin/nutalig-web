...
import { downloadTemplate, uploadProductPrice } from "services/Product/product-api";

export interface UploadProductPriceDialogProps {
    open: boolean,
    onClose: (jobId?: string) => void;
}

export default function UploadProductPriceDialog(props: UploadProductPriceDialogProps): JSX.Element {
    ...
    const handleConfirmUpload = async () => {
        ...
        toast.promise(uploadProductPrice(formData),
            {
                ...
                success: (response) => {
                    ...
                    setIsLoading(false);
                    onClose(jobId);
                    return t('productManagement.productPrice.importSuccess')
                },
                error: (error) => t('productManagement.productPrice.importFailed') + error.message,
            }
        ).finally(() => {
            setIsLoading(false);
        ...
        <Dialog open={open} maxWidth="xs" fullWidth aria-labelledby="form-dialog-title" >
            <DialogTitle id="form-dialog-title">
                <Box display="flex" justifyContent="space-between" alignItems="center">
                    <Typography variant="h6">
                        {t('productManagement.productPrice.uploadData')}
                    </Typography>
                    <Typography
                        variant="subtitle2"
                        onClick={() => {
                            handleDownloadTemplate('TEMPL-00005');
                        }}
                        style={{ cursor: 'pointer', color: '#1976d2' }} // ทำให้เหมือนลิงก์
                    >
                        {t('productManagement.productPrice.downloadTemplate')}
                    </Typography>
                </Box>
            </DialogTitle>
            <DialogContent>
            ...
            <DialogActions>
                ...
                <Button
                    ...
                    onClick={() => {
                        setTitle(t('productManagement.productPrice.confirmUploadTitle'));
                        setMsg(t('productManagement.productPrice.confirmUploadMsg'));
                        setAction('SUBMIT');

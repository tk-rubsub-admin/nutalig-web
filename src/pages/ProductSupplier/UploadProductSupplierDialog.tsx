...
import { downloadTemplate, uploadProductSupplier } from "services/Product/product-api";

export interface UploadProductSupplierDialogProps {
    open: boolean,
    onClose: (jobId?: string) => void;
}

export default function UploadProductSupplierDialog(props: UploadProductSupplierDialogProps): JSX.Element {
    ...
    const handleConfirmUpload = async () => {
        ...
        toast.promise(uploadProductSupplier(formData),
            {
                ...
                success: (response) => {
                    ...
                    setIsLoading(false);
                    onClose(jobId);
                    return t('productManagement.productSupplier.importSuccess')
                },
                error: (error) => t('productManagement.productSupplier.importFailed') + error.message,
            }
        ).finally(() => {
        ...
        <Dialog open={open} maxWidth="xs" fullWidth aria-labelledby="form-dialog-title" >
            <DialogTitle id="form-dialog-title">
                <Box display="flex" justifyContent="space-between" alignItems="center">
                    <Typography variant="h6">
                        {t('productManagement.productSupplier.uploadData')}
                    </Typography>
                    <Typography
                        variant="subtitle2"
                        onClick={() => {
                            handleDownloadTemplate('TEMPL-00001');
                        }}
            ...
            <DialogActions>
                ...
                <Button
                    ...
                    onClick={() => {
                        setTitle(t('productManagement.productSupplier.confirmUploadTitle'));
                        setMsg(t('productManagement.productSupplier.confirmUploadMsg'));
                        setAction('SUBMIT');

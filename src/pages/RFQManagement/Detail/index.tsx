...
import RequestRFQCustomerTransferDialog from 'dialogs/RFQManagement/Detail/RequestRFQCustomerTransferDialog';
...
function isVideoFile(file?: RFQFileResource | null): boolean {
  ...
  return (
    mimeType.startsWith('video/') || /\.(mp4|webm|mov|m4v|ogv|ogg|avi|mkv)(?:$|[?#])/.test(fileName)
  );
}
...
export default function RFQDetail(): ReactElement {
  ...
  const [visibleCustomerTransferDialog, setVisibleCustomerTransferDialog] = useState(false);
  ...
  const [visibleCustomerTransferDetailDialog, setVisibleCustomerTransferDetailDialog] = useState(false);
  const [customerTransferRejectReason, setCustomerTransferRejectReason] = useState('');
  const [isCustomerTransferActionSubmitting, setIsCustomerTransferActionSubmitting] = useState(false);
  ...
  const [lastShownRequestedInformationKey, setLastShownRequestedInformationKey] = useState<
    string | null
  >(null);
  const [selectedConfirmRfqTierKeys, setSelectedConfirmRfqTierKeys] = useState<string[]>([]);
  ...
  const canTransferRfqCustomer = hasPermission(PERMISSIONS.RFQ_CUSTOMER_TRANSFER);
  const canActOnCustomerTransfer = hasPermission(PERMISSIONS.RFQ_CUSTOMER_TRANSFER_APPROVE);
  ...
  const handleOpenRequestSpecialPriceDialog = () => {
    const targetPrices = (rfq?.details || []).reduce<Record<number, string>>((prices, detail) => {
      detail.tiers.forEach((tier) => {
        const targetPrice = tier.targetPrice;
        prices[tier.id] =
          targetPrice === null || targetPrice === undefined ? '' : formatTargetPrice(targetPrice);
      });
      return prices;
    }, {});
  ...
  const latestSupplierQuoteLeadTimes = useMemo(
    () =>
      [...(latestSupplierQuote?.leadTimes || [])].sort(
  ...
  const toggleConfirmRfqTierKey = (rowKey: string) => {
    setSelectedConfirmRfqTierKeys((currentKeys) =>
  ...
  const handleConfirmRfqPrice = () => {
    if (!selectedConfirmRfqTierKeys.length) {
      toast.error('กรุณาเลือกรายการใบเสนอราคาที่ต้องการใช้สำหรับคอนเฟิร์มราคา');
      return;
    }

    const selectedRows = selectedConfirmRfqTierKeys
    ...
    history.push(
      `${ROUTE_PATHS.SALE_ORDER_CREATE_FROM_RFQ.replace(
        ':rfqId',
        params.id
  ...
  const canEditPictures =
    isAllowUploadAttachment && pictureResources.length < config.maxRfqPictures;
  ...
  const canRequestSpecialPriceAction = hasPermission(PERMISSIONS.RFQ_EDIT) && rfq?.status === 'NEW';
  ...
  const handleRequestSpecialPrice = async () => {
    if (!params.id) {
      return;
    }
  ...
  return (
    <Page>
        ...
        <Stack
          ...
          {rfq?.status ? (
            <Stack direction="row" spacing={1} alignItems="center" useFlexGap flexWrap="wrap">
              ...
              {rfq.customerTransferApproval ? (
                <Chip
                  label={
                    rfq.customerTransferApproval.status === 'APPROVED'
                      ? 'เปลี่ยนลูกค้าอนุมัติแล้ว'
                      : rfq.customerTransferApproval.status === 'REJECTED'
                        ? 'คำขอเปลี่ยนลูกค้าไม่อนุมัติ'
                        : 'เปลี่ยนลูกค้ารออนุมัติ'
                  }
                  size="small"
                  clickable
                  onClick={() => setVisibleCustomerTransferDetailDialog(true)}
                  sx={{
                    height: 28,
                    backgroundColor: rfq.customerTransferApproval.status === 'APPROVED' ? '#dcfce7' : rfq.customerTransferApproval.status === 'REJECTED' ? '#e2e8f0' : '#fff7ed',
                    color: rfq.customerTransferApproval.status === 'APPROVED' ? '#15803d' : rfq.customerTransferApproval.status === 'REJECTED' ? '#475569' : '#c2410c',
                    border: rfq.customerTransferApproval.status === 'APPROVED' ? '1px solid #22c55e33' : rfq.customerTransferApproval.status === 'REJECTED' ? '1px solid #94a3b833' : '1px solid #fb923c33',
                    fontWeight: 700,
                    alignSelf: 'center',
        ...
        <Stack spacing={{ xs: 1.5, sm: 2 }}>
              ...
              <>
                ...
                <Menu
                  ...
                  {canReviewSpecialPriceAction ? (
                    <MenuItem onClick={handleOpenRequestSpecialPriceDialog} sx={{ width: '100%' }}>
                      <ListItemIcon>
                  ...
                  {isSalesPermission ? (
                    rfq?.customer ? (
                      canTransferRfqCustomer ? (
                        <MenuItem
                          disabled={rfq.customerTransferApproval?.status === 'PENDING'}
                          onClick={() => setVisibleCustomerTransferDialog(true)}
                          sx={{ width: '100%' }}>
                          <ListItemIcon>
                            <PersonAdd fontSize="small" />
                          </ListItemIcon>
                          <ListItemText primary="เปลี่ยนลูกค้า" />
                        </MenuItem>
                      ) : null
                    ) : (
                      <MenuItem
                        onClick={() => setVisibleLinkCustomerDialog(true)}
                        sx={{ width: '100%' }}>
                        <ListItemIcon>
                          <PersonAdd fontSize="small" />
                        </ListItemIcon>
                  ...
                  {/* {canDownloadSalesOrderAction ? (
                    <MenuItem
                      ...
                      <ListItemText
                        primary={t('rfqManagement.detail.actions.downloadSalesOrder')}
                      />
                    </MenuItem>
                  ) : null} */}
                  {canCloseRfqAction ? (
                    ...
                    {!isLast ? (
                      <Box
                        sx={{
                          ...
                          borderRadius: 999,
                          backgroundColor:
                            rfqStatusTimelineReachedIndex >= index + 1
                              ? reachedColor
                              : pendingColor,
                          mb: { xs: 4.6, sm: 3.4 }
                        }}
          ...
          <Box
            ...
            <Tabs
              ...
              <Tab value="detail" label={t('rfqManagement.detail.tabs.detail')} />
              <Tab value="history" label={t('rfqManagement.detail.tabs.history')} />
            </Tabs>
          </Box>
              ...
              {quotationOptions.length > 0 ? (
                <Box
                  ...
                  <Stack
                    ...
                    {activeQuotationNo ? (
                      <Chip size="small" label={`ใช้งาน: ${activeQuotationNo}`} />
                    ) : null}
                  </Stack>
                          ...
                          <Stack
                            ...
                            <Stack spacing={0.75}>
                              <Stack
                                ...
                                {isLatest ? (
                                  <Chip size="small" color="success" label="ล่าสุด" />
                                ) : null}
                                {isActive ? (
                                  <Chip size="small" color="primary" label="ใช้งาน" />
                                ) : null}
                              </Stack>
              ...
              <CollapsibleWrapper
                ...
                <Grid container spacing={1}>
                  <GridTextField item xs={12} sm={6}>
                    <TextField
                      fullWidth
                      label={t('rfqManagement.column.id')}
                      value={rfq?.id || ''}
                  ...
                  <GridTextField item xs={12} sm={6}>
                    <TextField
                      fullWidth
                      label={t('rfqManagement.column.requestedDate')}
                      value={
                        rfq?.requestedDate
                        ...
                        endAdornment: rfq?.referenceRfqId ? (
                          <InputAdornment position="end">
                            <IconButton
                              edge="end"
                              size="small"
                              aria-label={t('rfqManagement.detail.actions.openReferenceRfq')}
                              onClick={() =>
                                window.open(
                                  ROUTE_PATHS.RFQ_DETAIL.replace(':id', rfq.referenceRfqId || ''),
                                  '_blank',
                                  'noopener,noreferrer'
                                )
                              }>
                  ...
                  <GridTextField item xs={12} sm={6}>
                    <TextField
                      ...
                      InputLabelProps={{ shrink: true }}
                      disabled={
                        !isSalesPermission || !formik.values.salesId || isProcurementFetching
                      }>
                                  ...
                                  <TextField
                                    ...
                                    onBlur={() =>
                                      formik.setFieldTouched(
                                        `requestedMoqs.${index}.targetPrice`,
                                        true
                                      )
                                    }
                  ...
                  <GridTextField item xs={12} sm={6}>
                    <TextField
                      fullWidth
                      label={t('rfqManagement.detail.fields.createdDate')}
                      value={
                        rfq?.createdDate
                  ...
                  <GridTextField item xs={12} sm={6}>
                    <TextField
                      fullWidth
                      label={t('rfqManagement.detail.fields.updatedDate')}
                      value={
                        rfq?.updatedDate
                  ...
                  <GridTextField item xs={12}>
                    <Typography variant="subtitle1" fontWeight={600}>
                      {t('rfqManagement.form.attachments')}
                    </Typography>
                  </GridTextField>
                  <GridTextField item xs={12}>
                                ...
                                <Grid container spacing={1.5} sx={{ mt: 0.5, maxWidth: 860 }}>
                                  <Grid item xs={12} md={12}>
                                    <TextField
                                      fullWidth
                                      size="small"
                                      label={t('rfqManagement.detail.fields.spec')}
                                      value={detail.spec}
                                      InputLabelProps={{ shrink: true }}
                                  ...
                                  <Grid item xs={12} md={12}>
                                    <TextField
                                      fullWidth
                                      size="small"
                                      label={t('rfqManagement.detail.fields.remark')}
                                      value={detail.remark || ''}
                                      InputLabelProps={{ shrink: true }}
                                      onChange={(event) =>
                                        handleDraftDetailChange(
                                          ...
                                          <TableRow key={tier.id}>
                                            ...
                                            <TableCell sx={{ minWidth: 150 }}>
                                              <TextField
                                                ...
                                                <MenuItem value="LAND">
                                                  {getShippingMethodLabel('LAND')}
                                                </MenuItem>
                                                <MenuItem value="SEA">
                                                  {getShippingMethodLabel('SEA')}
                                                </MenuItem>
                                              </TextField>
                                            </TableCell>
                                            <TableCell sx={{ minWidth: 150 }}>
                                        ...
                                        <Grid container spacing={1}>
                                          <Grid item xs={12}>
                                            <TextField
                                              ...
                                              size="small"
                                              type="number"
                                              label={t('rfqManagement.form.requestedMoq')}
                                              value={tier.quantity}
                                              error={Boolean(
                                                detailError.tierErrors?.[tier.id]?.quantity
                                              )}
                                          ...
                                          <Grid item xs={12}>
                                            <TextField
                                              ...
                                              size="small"
                                              type="number"
                                              label={t('rfqManagement.detail.fields.productPrice')}
                                              value={tier.productPrice}
                                              error={Boolean(
                                                detailError.tierErrors?.[tier.id]?.productPrice
                                              )}
                  ...
                  <Box
                    ...
                    <Typography variant="body1" fontWeight={600}>
                      {isSupplierQuotesFetching
                        ? 'กำลังโหลดข้อมูล Lead Time'
                        : 'ยังไม่มีข้อมูล Lead Time'}
                    </Typography>
                  </Box>
                )}
              </CollapsibleWrapper>
              <CollapsibleWrapper
                ...
                {additionalCosts.length || draftAdditionalCosts.length ? (
                      ...
                      <TableHead>
                        <TableRow
                          ...
                          <TableCell align="center" width="40%">
                            {t('rfqManagement.detail.additionalCost.name')}
                          </TableCell>
                          <TableCell>
                            {t('rfqManagement.detail.additionalCost.description')}
                          </TableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
                          ...
                          <TableRow
                            ...
                            <TableCell sx={{ fontWeight: 600 }}>
                              <TextField
                                fullWidth
                                size="small"
                                placeholder={t('rfqManagement.detail.additionalCost.name')}
                            ...
                            <TableCell>
                              <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1}>
                                <TextField
                                  fullWidth
                                  size="small"
                                  placeholder={t('rfqManagement.detail.additionalCost.value')}
                                ...
                                <TextField
                                  fullWidth
                                  size="small"
                                  placeholder={t('rfqManagement.detail.additionalCost.unit')}
                  ...
                  <Box
                    ...
                    <Typography variant="body1" fontWeight={600}>
                      {t('rfqManagement.detail.additionalCost.empty')}
                    </Typography>
                  </Box>
                )}
              </CollapsibleWrapper>
      ...
      <ConfirmDialog
        ...
        title={t('rfqManagement.detail.dialogs.saveOptionsTitle')}
        message={t('rfqManagement.detail.dialogs.saveOptionsMessage')}
        confirmText={t('button.confirm')}
        cancelText={t('button.cancel')}
        isShowCancelButton
        isShowConfirmButton
      ...
      <ConfirmDialog
        ...
        title={t('rfqManagement.detail.dialogs.deleteOptionTitle')}
        message={t('rfqManagement.detail.dialogs.deleteOptionMessage', {
          optionName:
            selectedDetailToDelete?.optionName || t('rfqManagement.detail.sections.options')
        })}
        confirmText={t('button.confirm')}
        cancelText={t('button.cancel')}
        isShowCancelButton
      ...
      <ConfirmDialog
        ...
        title={t('rfqManagement.detail.dialogs.deleteAdditionalCostTitle')}
        message={t('rfqManagement.detail.dialogs.deleteAdditionalCostMessage', {
          description:
            selectedAdditionalCostToDelete?.description ||
            t('rfqManagement.detail.sections.additional')
        })}
        confirmText={t('button.confirm')}
        cancelText={t('button.cancel')}
        isShowCancelButton
      ...
      <ConfirmDialog
        ...
        message={t('rfqManagement.detail.dialogs.rejectMessage', { id: rfq?.id || '' })}
        confirmText={t('button.confirm')}
        cancelText={t('button.cancel')}
        isShowCancelButton
        isShowConfirmButton
      ...
      <ConfirmDialog
        ...
        title={t('rfqManagement.detail.dialogs.closeTitle')}
        message={t('rfqManagement.detail.dialogs.closeMessage', { id: rfq?.id || '' })}
        confirmText={t('button.confirm')}
        cancelText={t('button.cancel')}
        isShowCancelButton
        isShowConfirmButton
      ...
      <Dialog
        ...
        <DialogContent dividers>
          ...
          <TextField
            ...
            InputLabelProps={{ shrink: true }}
            placeholder={t('rfqManagement.detail.dialogs.reasonPlaceholder')}
          />
        </DialogContent>
        <DialogActions>
          <Button
                    ...
                    setRequestSpecialPriceTargetPriceErrors((previous) => ({
                      ...
                      ...Object.fromEntries(
                        (rfq?.details || [])
                          ...
                          .filter(
                            (candidateTier) =>
                              Number(candidateTier.quantity) === Number(tier.quantity)
                          )
      ...
      <Dialog
        ...
        <DialogTitle>{t('rfqManagement.detail.actions.confirmPrice')}</DialogTitle>
        <DialogContent dividers>
          <Stack spacing={2}>
            <Typography variant="body2" color="text.secondary">
            ...
            {isQuotationFetching ? (
              <Box
                ...
                <Typography variant="body1" fontWeight={600}>
                  {t('rfqManagement.detail.dialogs.loadingQuotation')}
                </Typography>
              </Box>
                  ...
                  <TableHead>
                    <TableRow
                      ...
                      <TableCell align="center" width={64}>
                        {t('rfqManagement.detail.quotationTable.select')}
                      </TableCell>
                      <TableCell>{t('rfqManagement.detail.quotationTable.item')}</TableCell>
                      <TableCell align="center">
                        {t('rfqManagement.detail.quotationTable.quantity')}
                      </TableCell>
                      <TableCell align="center">
                        {t('rfqManagement.detail.quotationTable.shippingMethod')}
                      </TableCell>
                      <TableCell align="center">
                        {t('rfqManagement.detail.quotationTable.unitPrice')}
                      </TableCell>
                      <TableCell align="center">
                        {t('rfqManagement.detail.quotationTable.amount')}
                      </TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
              ...
              <Box
                ...
                <Typography variant="body1" fontWeight={600}>
                  {t('rfqManagement.detail.dialogs.noQuotation')}
                </Typography>
              </Box>
            )}
          </Stack>
        ...
        <DialogActions>
          ...
          <Button
            ...
            className="btn-emerald-green"
            variant="contained"
            disabled={!selectedConfirmRfqTierKeys.length}>
            {t('button.confirm')}
          </Button>
        </DialogActions>
      </Dialog>
      <Dialog
        ...
        maxWidth="sm"
        fullWidth>
        <DialogTitle>{t('rfqManagement.detail.urgent.title')}</DialogTitle>
        <DialogContent dividers>
          <Stack spacing={2}>
            <Box>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>
                {t('rfqManagement.detail.urgent.status')}
              </Typography>
              <Typography variant="body1" fontWeight={700}>
                {rfq?.urgentApproval?.status === 'APPROVED'
                  ? t('rfqManagement.detail.urgent.statusApproved')
                  : rfq?.urgentApproval?.status === 'REJECTED'
                    ? t('rfqManagement.detail.urgent.statusRejected')
                    : t('rfqManagement.detail.urgent.statusPending')}
              </Typography>
            </Box>
            <Box>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>
                {t('rfqManagement.detail.urgent.requestReason')}
              </Typography>
              <Typography variant="body1" sx={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                {rfq?.urgentApproval?.requestReason || '-'}
              </Typography>
            </Box>
            <Box>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>
                {t('rfqManagement.detail.urgent.rejectReason')}
              </Typography>
              <Typography variant="body1" sx={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
      ...
      <Dialog
        open={visibleCustomerTransferDetailDialog}
        onClose={() => {
          if (!isCustomerTransferActionSubmitting) {
            setVisibleCustomerTransferDetailDialog(false);
            setCustomerTransferRejectReason('');
          }
        }}
        maxWidth="sm"
        fullWidth>
        <DialogTitle>รายละเอียดคำขอเปลี่ยนลูกค้า</DialogTitle>
        <DialogContent dividers>
          <Stack spacing={2}>
            <Box>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>สถานะ</Typography>
              <Typography variant="body1" fontWeight={700}>
                {rfq?.customerTransferApproval?.status === 'APPROVED' ? 'อนุมัติแล้ว' : rfq?.customerTransferApproval?.status === 'REJECTED' ? 'ไม่อนุมัติ' : 'รออนุมัติ'}
              </Typography>
            </Box>
            <Box>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>ลูกค้าปัจจุบัน</Typography>
              <Typography variant="body1">{rfq?.customerTransferApproval?.payload?.currentCustomerName || '-'}</Typography>
            </Box>
            <Box>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>เปลี่ยนเป็นลูกค้า</Typography>
              <Typography variant="body1">{rfq?.customerTransferApproval?.payload?.targetCustomerName || '-'}</Typography>
            </Box>
            <Box>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>เหตุผลการเปลี่ยนลูกค้า</Typography>
              <Typography variant="body1" sx={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{rfq?.customerTransferApproval?.requestReason || rfq?.customerTransferApproval?.payload?.reason || '-'}</Typography>
            </Box>
            <Box>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>เหตุผลที่ไม่อนุมัติ</Typography>
              <Typography variant="body1" sx={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{rfq?.customerTransferApproval?.rejectReason || '-'}</Typography>
            </Box>
            {rfq?.customerTransferApproval?.status === 'PENDING' && canActOnCustomerTransfer ? (
              <TextField
                required
                fullWidth
                multiline
                ...
                value={customerTransferRejectReason}
                onChange={(event) => setCustomerTransferRejectReason(event.target.value)}
                InputLabelProps={{ shrink: true }}
              />
            ) : null}
          </Stack>
          ...
          {rfq?.customerTransferApproval?.status === 'PENDING' && canActOnCustomerTransfer ? (
            <>
              <Button
                variant="contained"
                color="error"
                disabled={!customerTransferRejectReason.trim() || isCustomerTransferActionSubmitting}
                onClick={async () => {
                  if (!params.id) return;
                  setIsCustomerTransferActionSubmitting(true);
                  try {
                    await toast.promise(
                      rejectRFQCustomerTransfer(params.id, { reason: customerTransferRejectReason.trim() }),
                      { loading: 'กำลังไม่อนุมัติคำขอเปลี่ยนลูกค้า', success: 'ไม่อนุมัติคำขอเปลี่ยนลูกค้าแล้ว', error: 'ไม่สามารถไม่อนุมัติคำขอเปลี่ยนลูกค้าได้' }
                    );
                    setVisibleCustomerTransferDetailDialog(false);
                    setCustomerTransferRejectReason('');
                    await refetchRFQ();
                  } finally { setIsCustomerTransferActionSubmitting(false); }
                }}>
                ไม่อนุมัติ
              </Button>
              <Button
                variant="contained"
                color="success"
                disabled={isCustomerTransferActionSubmitting}
                onClick={async () => {
                  if (!params.id) return;
                  setIsCustomerTransferActionSubmitting(true);
                  try {
                    await toast.promise(approveRFQCustomerTransfer(params.id), { loading: 'กำลังอนุมัติคำขอเปลี่ยนลูกค้า', success: 'อนุมัติคำขอเปลี่ยนลูกค้าแล้ว', error: 'ไม่สามารถอนุมัติคำขอเปลี่ยนลูกค้าได้' });
                    setVisibleCustomerTransferDetailDialog(false);
                    await refetchRFQ();
                  } finally { setIsCustomerTransferActionSubmitting(false); }
                }}>
      ...
      />
      <RequestRFQCustomerTransferDialog
        open={visibleCustomerTransferDialog}
        rfq={rfq}
        onClose={() => setVisibleCustomerTransferDialog(false)}
        onSubmitted={refetchRFQ}
      />
      <Dialog

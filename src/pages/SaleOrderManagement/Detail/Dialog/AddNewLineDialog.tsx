...
// 👇 สมมุติคุณมี bulk API แบบนี้ (ปรับตามจริง)
...
type LineForm = { item: ProductDto | null; qty: number | null; remark: string, isClaimed: boolean };
const makeEmptyLine = (): LineForm => ({ item: null, qty: null, remark: '', isClaimed: false });
...
export default function AddNewLineDialog({ open, poId, onClose }: AddNewLineDialogProps) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            const payload = values.lines
                ...
                .map(l => ({
                    itemSku: l.item!.productSku,
                    itemName: l.item!.productNameTh,
                    qty: l.qty!,
                    remark: l.remark ?? '',
    ...
    // ————————————————— UI —————————————————
    return (
        <Dialog open={open} maxWidth="xl" fullWidth aria-labelledby="form-dialog-title">
                                            ...
                                            >
                                                ...
                                                <Grid item xs={9} sm={3}>
                                                    <TextField
                                                        ...
                                                        value={line.remark}
                                                        onChange={(e) =>
                                                            formik.setFieldValue(`lines[${idx}].remark`, e.target.value)
                                                        }
                                                        fullWidth
                                                        InputLabelProps={{ shrink: true }}
                                                        helperText={' '}
                                                        sx={{ '& .MuiFormHelperText-root': { minHeight: 22 } }}
                                                    />
                                                </Grid>
                                                ...
                                                <Grid item xs={6} sm={1}>
                                                    <CustomSwitch
                                                        ...
                                                        checked={line.isClaimed || false}
                                                        label={'สินค้าเคลม'}
                                                        onChange={(e) => {
                                                            formik.setFieldValue(`lines[${idx}].isClaimed`, e);
                                                            if (e) {
                                                                formik.setFieldValue(`lines[${idx}].remark`, 'สินค้าเคลม');
                                                            } else {
                                                                formik.setFieldValue(`lines[${idx}].remark`, '');
                                                            }
                                                        }}
                                                    />
                                                </Grid>
